...
if (defined('ALPHA_INTEGRATION_MODE') && ALPHA_INTEGRATION_MODE === 'save') {
    ...
    $exists = $wpdb->get_var($wpdb->prepare("SELECT id FROM $table WHERE name = %s", 'drip'));

    if ($exists) {
        $wpdb->update($table, ['data' => $data], ['id' => $exists]);
    } else {
        $wpdb->insert($table, ['name' => 'drip', 'data' => $data]);
    }
...
if (defined('ALPHA_INTEGRATION_MODE') && ALPHA_INTEGRATION_MODE === 'validate') {
    ...
    $response = wp_remote_get('https://api.getdrip.com/v2/accounts', [
        'headers' => [
    ...
    if (is_wp_error($response)) {
        wp_send_json_error(['message' => 'Erro ao conectar com a API do Drip.']);
    }
